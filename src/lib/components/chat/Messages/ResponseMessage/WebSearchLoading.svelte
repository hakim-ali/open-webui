...
<div class="status {fadeState}" aria-live="polite" aria-atomic="true">
  <span class="shimmer-text" style="color: {text.includes('Searched') && text.includes('Shortlisted') ? '#23282E' : '#666D7A'}">
    {#if text === 'Just a sec...'}
      {$i18n.t('Just a sec...')}
    {:else if text === 'Searching on web...'}
...
<style>
  ...
  .status { display:flex; align-items:center; gap:.5rem; opacity:0; }
  .status.in  { opacity:1; transition:opacity 180ms ease; }
  .status.out { opacity:0; transition:opacity 800ms ease; }
</style>
