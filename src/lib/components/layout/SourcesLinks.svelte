	...
	async function extractLinksFromMessage() {
		...
		try {
			...
			const uniqueLinksMap = new Map<
				string,
				{ href: string; text: string; title: string; domain: string; favicon: string }
			>();
			...
			// Convert map back to array and add loading state with citation numbers
			extractedLinks = Array.from(uniqueLinksMap.values()).map((link, index) => ({
				...link,
				loading: true,
			...
			console.log(
				`Found ${messageLinks.length} total links, ${extractedLinks.length} unique links for message ${$sourcesLinksMessageId}`
			);
				...
				extractedLinks.map(async (link, index) => {
					try {
						...
						extractedLinks[index] = {
							...
							loading: false,
							citationNumber: link.citationNumber
						};
						...
						<div class="flex-1 flex items-center justify-center p-6">
							<div class="text-center text-gray-500 dark:text-gray-400">
								...
								<p class="text-xs mt-1">
									{$i18n.t('Links from your conversation will appear here')}
								</p>
							</div>
						</div>
					{/if}
