<script lang="ts">
	...
	const setChatPlaceholderStyle = (theme: string) => {
		const styleId = 'chat-input p';
		...
		styleTag.textContent = `
				    ...
				    pointer-events: none;
				    position: absolute;
				    color: ${theme === 'dark' ? '#6B717C' : '#a5a6a9'};
				    line-height: 1.5;
				    white-space: nowrap;
				    overflow: hidden;
	...
	const applyTheme = (_theme: string) => {
		...
		if (metaThemeColor) {
			if (_theme.includes('system')) {
				...
				setChatPlaceholderStyle(_theme);
			}
		}
...
>
	...
	<slot name="content">
		...
		>
			<button
				class="flex justify-between items-center border-b border-gray-100 dark:border-transparent px-[16px] py-[11px] w-full transition cursor-pointer hover:bg-menu-hover"
